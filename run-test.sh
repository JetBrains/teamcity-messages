...
run() {
    ...
    $CMD 2>&1 | sed 's#File "[^"]*/#File "#g' | sed 's#instance at 0x.*>#instance at 0x????????>#' | perl -pe 's/line \d+/line LINE/g' >test-$NAME.output.tmp 
...
run pytest "py.test --teamcity test-pytest.py"
